...
The docker image file can be found at [Dockerhub](https://hub.docker.com/repository/docker/skirophorion/deepgem/general)
...
```
docker run --name deepgem --gpus '"device=0"' -it --rm  skirophorion/deepgem:latest /bin/bash
```
...
More data information are given in `./data/ReadMe.txt`.
